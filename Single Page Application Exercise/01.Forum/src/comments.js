...
const userDiv = comments.querySelector('.user-comment');
...
async function getPost(event) {
  ...
  const targetPost = await fetch(newUrl).then((response) => response.json());
...
async function postComment(event) {
  ...
  try {
    const response = await fetch(commentsUrl, {
      ...
      body: JSON.stringify({
        userId,
        postText, // Change from 'content' to 'postText'
        username,
        date: new Date(),
      }),
    });
